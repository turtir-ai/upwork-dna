...
function isRateLimited() {
  // Only detect actual HTTP 429 error pages, NOT normal pages with "rate" in job descriptions
  const title = (document.title || "").toLowerCase();
  if (title.includes("429") || title.includes("too many requests")) return true;

  // Check for Upwork-specific error page containers (these don't exist on normal search/detail pages)
  ...
  if (errorContainer) {
    ...
    if (errorText.includes("too many requests") || errorText.includes("rate limit") || errorText.includes("try again later")) {
      return true;
    }
  }

  // Fallback: if page is essentially empty (< 500 chars visible) AND mentions rate/429
  const bodyLen = (document.body && document.body.innerText || "").length;
  if (bodyLen < 500) {
    const bodyText = (document.body && document.body.innerText) || "";
    if (bodyText.includes("429") || bodyText.toLowerCase().includes("too many requests")) return true;
  }
...
const RUNTIME_RECOVERY_FLAG = "upwork_dna_runtime_recover_once";
const ENABLE_RUNTIME_AUTO_RECOVERY = true;

function clearRuntimeRecoveryFlag() {
  try {
    sessionStorage.removeItem(RUNTIME_RECOVERY_FLAG);
  } catch (error) {
    return;
  }
}
...
function scheduleRuntimeRecovery() {
  if (!ENABLE_RUNTIME_AUTO_RECOVERY) {
    return;
  }
  if (runtimeRecoveryScheduled) {
    return;
  }
  ...
  const alreadyRecovered = sessionStorage.getItem(RUNTIME_RECOVERY_FLAG) === "1";
  if (alreadyRecovered) {
    return;
  }
...
function safeSendRuntimeMessage(message, callback) {
  if (!hasRuntimeMessaging()) {
    ...
    if (typeof callback === "function") {
      callback(null, "runtime_unavailable");
    }
    return false;
  }
  ...
  try {
    chrome.runtime.sendMessage(message, (response) => {
      ...
      if (lastError) {
        if (typeof callback === "function") {
          callback(null, lastError.message || "runtime_error");
        }
        return;
      }
      if (typeof callback === "function") {
    ...
    if (typeof callback === "function") {
      callback(null, error.message || "runtime_exception");
    }
    return false;
  }
}
